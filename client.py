...
c1=socket.socket()
c1.bind(('localhost',9998))
connection_staus=False
for i in range(5):
    try:
        c1.connect(('localhost',9999))
        connection_staus=True
        break
    except ConnectionRefusedError:
        print('Peer is down, will retry in 5 seconds')
        sleep(5)
if connection_staus:
    ...
    peer_key=bytes(key2[0:len(key1)//2]+key1[len(key1)//2:])
    ...
    c1.close()
    print('Peer is offline, please try again later.')